"""Async REST transfer client for download and upload."""
...
class AsyncRestTransferClient:
    """Async streaming REST download and multipart upload."""
    ...
    def _client(self) -> httpx.AsyncClient:
        return self._cert_store.build_async_httpx_client(verify=self._verify)

    @retry_rest(max_attempts=3)
    async def download_to_file(self, url: str, target: Path) -> HttpTransferResult:
    ...
    @retry_rest(max_attempts=3)
    async def upload_multipart(self, url: str, file: Path, filename: str) -> RestUploadResult:
        async with self._client() as client:
            with open(file, "rb") as f:
                files = {
                    "fileupload": (filename, f, "application/octet-stream"),
                }
                response = await client.post(url, files=files, timeout=self._timeout)
