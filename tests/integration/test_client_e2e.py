...
async def test_full_download_upload_flow(
    mock_soap_gateway,
    mock_async_soap,
    mock_rest,
    mock_state,
    ...
    soap = MagicMock()
    mock_async_soap.return_value = soap
    ...
    soap.get_download_file_list_v4 = AsyncMock(
        return_value=MagicMock(query_timestamp=None, files=[])
    )
    files = await client.list_available_files(DownloadFilter())
    assert files == []
