...
def test_resume_pending_finishes_upload(repo: SqliteStateRepository, tmp_path: Path):
    ...
    soap.finish_upload_file_list_v2.return_value = [
        MagicMock(filename="pay.xml", hash="a" * 64, status=UploadFinishStatus.I, ticket_id="T1")
    ]
