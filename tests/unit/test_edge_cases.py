...
@respx.mock
@pytest.mark.asyncio
async def test_upload_multipart_malformed_json():
...
@respx.mock
@pytest.mark.asyncio
async def test_upload_multipart_malformed_schema():
...
@respx.mock
@pytest.mark.asyncio
async def test_download_connection_timeout(tmp_path: Path):
...
@respx.mock
@pytest.mark.asyncio
async def test_download_read_timeout(tmp_path: Path):
    route = respx.get("https://example.com/file").mock(
...
@respx.mock
@pytest.mark.asyncio
async def test_download_503_retryable():
...
@respx.mock
@pytest.mark.asyncio
async def test_upload_connection_timeout():
    route = respx.post("https://example.com/upload").mock(
        side_effect=httpx.ConnectTimeout("Connection timed out")
    )
    client = AsyncRestTransferClient(cert_store=_store())
